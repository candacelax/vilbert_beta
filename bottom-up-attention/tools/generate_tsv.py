...
def load_image_ids(split_name, group_id, total_group, data_dir, image_ids=None):
    ...
    if split_name == 'coco_train2014':
      ...
      df = open_tsv('{}/Validation_GCC-1.1.0-Validation.tsv'.format(data_dir), 'validation')
      total_num = len(df)
      per_num = int(np.ceil(total_num / total_group))
      current_df = df[int(group_id * per_num):int((group_id+1)*per_num)]
      for i, img in enumerate(current_df.iterrows()):
        ...
        im_name = _file_name(img[1])
        image_id = np.uint64(im_name.split('/')[1])
        filepath = os.path.join(data_dir, im_name)
        split.append((filepath, image_id))
...
def parse_args():
    ...
    parser.add_argument('--data_dir', type=str,
                        help='path to data parent directory')
    parser.add_argument(
        '--total_group',
...
def generate_tsv(gpu_id, prototxt, weights, image_ids, outfile):
    ...
    if len(missing) > 0:
        ...
        with open(outfile, 'ab') as tsvfile:
            ...
            count=0
            num_not_found=0
            print('Running through images', len(image_ids))
            for im_file,image_id in image_ids:
                if image_id in missing:
                    ...
                    _t['misc'].tic()
                    detections = get_detections_from_im(net, im, image_id)
        ...
        print('Loaded {} of {} missing images'.format(count, len(missing)))
        print('{} images were not found'.format(num_not_found))
...
if __name__ == '__main__':
    ...
    image_ids = load_image_ids(args.data_split, args.group_id, args.total_group, args.data_dir)
